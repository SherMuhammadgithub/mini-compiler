    ...
    fn node(&mut self, n: &AstNode) {
        match &n.kind {
            NodeKind::Program {
                ...
                for name in params {
                    let _ = self.sym.insert(make_entry(
                        name,
                        SymbolKind::Variable,
                        PascalType::Integer,
                        0,
            ...
            NodeKind::VarDecl { names, ty } => {
                ...
                for name in names {
                    ...
                    if let Err(err) = self.sym.insert(e) {
            ...
            NodeKind::FunctionDecl {
                ...
                let level = self.sym.current_level();
                if let Err(e) = self.sym.insert(make_entry(
                    name,
                    SymbolKind::Function,
                    ret,
                    level,
                ...
                self.dumps.push(ScopeDump {
                    level: level + 1,
                    entries: exited,
                });
            }

            NodeKind::ProcedureDecl {
                name,
                params,
                declarations,
                body,
            ...
            NodeKind::Assignment { target, value } => {
                ...
                if !compatible(&target_type, &value_type) {
                    self.errors.push(type_err(
                        &format!(
                            "cannot assign {} to {}",
                            pascal_name(&value_type),
                            pascal_name(&target_type)
                        ),
                        n.span.line,
                        n.span.column,
                    ));
            ...
            NodeKind::ProcedureCall { name, args } => {
                if !is_builtin(name) {
                    if self.sym.lookup(name).is_none() {
                        self.errors
                            .push(undeclared(name, n.span.line, n.span.column));
                    } else {
        ...
        match &n.kind {
            ...
            NodeKind::Variable { name, index } => {
                let base = self
                    .sym
                    .lookup(name)
                    .map(|e| e.pascal_type.clone())
                    .unwrap_or(PascalType::Integer);
                if index.is_some() {
                    match base {
                        PascalType::Array { element, .. } => *element,
                        t => t,
                    }
                } else {
                    base
...
/// Phase 9 entry point: run the checker and return symbol table information.
pub fn symbol_table_analyze(source: &str) -> crate::symbol_table::SymbolTableOutput {
    let rd_out = rd_parser::parse(source);
    let ast = rd_out.ast;
    let mut checker = Checker::new(rd_out.errors);
    if let Some(ref node) = ast {
    ...
    crate::symbol_table::SymbolTableOutput {
        entries: checker.sym.snapshot(),
        errors: checker.errors,
...
fn to_pascal_type(n: &AstNode) -> PascalType {
    match &n.kind {
        ...
        NodeKind::TypeArray { low, high, element } => PascalType::Array {
            low: *low,
            high: *high,
            element: Box::new(to_pascal_type(element)),
        },
        _ => PascalType::Integer,
    }
}

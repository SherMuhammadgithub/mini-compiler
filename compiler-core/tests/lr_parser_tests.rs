// Integration tests for the LALR(1) shift-reduce parser (Phase 8).
use compiler_core::lr_parser::parse;
...
#[test]
fn trace_first_step_is_shift() {
    let out = parse("program p ( x ) ; begin end .");
    assert!(
        out.trace[0].action.starts_with("Shift"),
        "first LR step should be a shift"
    );
}
